...
public class RegistrationSenderBehavior extends WakerBehaviour {
    ...
    /**
     * @param agent     - the agent containing this behavior.
     * @param timeout   - the time after which to send the registration message.
     * @param parentAID - the agent to send the registration message to.
     */
    ...
    @Override
    public int onEnd() {
        System.out.println("[REGISTRATION] Agent " + myAgent.getAID().getLocalName() +
				" has sent registration message to " + parentAID.getLocalName());
        return super.onEnd();
    }
